...
from modules.gui.gui_utils import (
    add_list_items,
    disable_incompatible_checkbox,
    disable_incompatible_element,
    move_list_items,
    order_list_items,
    remove_list_items,
    select_checkboxes,
...
def default_english_order(
    main_window: Any, region_order_default: list[str], regions_list: str
) -> None:
    """
...
def init_gui_global(
    main_window: Any, dat_details: dict[str, dict[str, str]], config: Config
) -> None:
    """
    ...
    try:
        gui_split_left = int(
            get_config_value(config.user_gui_settings, 'gui split left', '0', False)
        )
    except Exception:
        pass
    ...
    try:
        gui_split_right = int(
            get_config_value(config.user_gui_settings, 'gui split right', '0', False)
        )
    except Exception:
        pass
    ...
    if darkdetect.isDark():
        main_window.ui.labelGlobalOverride.setText(
            main_window.ui.labelGlobalOverride.text().replace('color:#0000ff', 'color:#47aae9')
        )
        main_window.ui.labelGlobalFilters.setText(
            main_window.ui.labelGlobalFilters.text().replace('color:#0000ff', 'color:#47aae9')
        )
        main_window.ui.labelGlobalLocalizeNames.setText(
            main_window.ui.labelGlobalLocalizeNames.text().replace('color:#0000ff', 'color:#47aae9')
        )
        main_window.ui.labelSystemOverride.setText(
            main_window.ui.labelSystemOverride.text().replace('color:#0000ff', 'color:#47aae9')
        )
        main_window.ui.labelSystemFilters.setText(
            main_window.ui.labelSystemFilters.text().replace('color:#0000ff', 'color:#47aae9')
        )
        main_window.ui.labelSystemLocalizeNames.setText(
            main_window.ui.labelSystemLocalizeNames.text().replace('color:#0000ff', 'color:#47aae9')
        )
    ...
    main_window.ui.listWidgetGlobalAvailableRegions.addItems(
        sorted([x for x in region_order_default if x not in region_order_user])
    )
    ...
    main_window.ui.listWidgetGlobalAvailableLanguages.addItems(
        sorted([x for x in config.languages if x not in languages_user])
    )
    ...
    main_window.ui.listWidgetGlobalLocalizationAvailableLanguages.addItems(
        sorted([x for x in config.languages if x not in localization_user])
    )

    # Add video standards to the video list
    if config.video_order_user:
        main_window.ui.listWidgetGlobalVideoStandards.addItems(list(config.video_order_user))
    else:
        main_window.ui.listWidgetGlobalVideoStandards.addItems(list(config.video_order_default))

    # Apply other settings from user-config.yaml
    if config.global_exclude:
        main_window.ui.textEditGlobalExclude.setText('\n'.join(config.global_exclude))
    if config.global_include:
        main_window.ui.textEditGlobalInclude.setText('\n'.join(config.global_include))
    if config.global_filter:
        main_window.ui.textEditGlobalFilterInclude.setText('\n'.join(config.global_filter))
    if config.user_prefix:
        main_window.ui.lineEditGlobalOptions1G1RPrefix.setText(config.user_prefix)
    if config.user_suffix:
        main_window.ui.lineEditGlobalOptions1G1RSuffix.setText(config.user_suffix)
    ...
    main_window.clone_list_metadata_url = get_config_value(
    ...
    output: list[dict[str, str]] = [
        x for x in config.user_gui_settings if 'output' in x and x != {'output': ''}
    ]

    if output:
        main_window.ui.labelGlobalOutputFolder.setText(output[0]['output'])
        main_window.output_folder = output[0]['output']

    if config.user_gui_settings:
        if 'replace' in config.user_gui_settings:
            main_window.ui.checkBoxGlobalReplaceInputDats.setChecked(True)
            main_window.ui.buttonGlobalChooseOutput.setEnabled(False)
            main_window.ui.labelGlobalSelectOutput.setEnabled(False)
            main_window.ui.labelGlobalOutputFolder.setEnabled(False)
    ...
    if config.user_gui_settings:
        ...
        if 'z' in config.user_gui_settings:
            main_window.ui.checkBoxGlobalOptionsModernPlatforms.setChecked(True)
        if 'y' in config.user_gui_settings:
            main_window.ui.checkBoxGlobalOptionsDemoteUnlicensed.setChecked(True)
        if 'nooverrides' in config.user_gui_settings:
            main_window.ui.checkBoxGlobalOptionsDisableOverrides.setChecked(True)
        if 'removesdat' in config.user_gui_settings:
        ...
        if 'listnames' in config.user_gui_settings:
            main_window.ui.checkBoxGlobalOptions1G1RNames.setChecked(True)
    ...
    compilations_mode = [x for x in config.user_gui_settings if 'compilations' in x]

    if compilations_mode:
        compilations_option = compilations_mode[0]['compilations']

        if compilations_option == 'i':
            main_window.ui.comboBoxGlobalChooseCompilationsMode.setCurrentIndex(1)
            main_window.ui.labelGlobalCompilationsExplanation.setText(const.COMPILATIONS_INDIVIDUAL)
        elif compilations_option == 'k':
            main_window.ui.comboBoxGlobalChooseCompilationsMode.setCurrentIndex(2)
            main_window.ui.labelGlobalCompilationsExplanation.setText(const.COMPILATIONS_KEEP)
        elif compilations_option == 'o':
            main_window.ui.comboBoxGlobalChooseCompilationsMode.setCurrentIndex(3)
            main_window.ui.labelGlobalCompilationsExplanation.setText(const.COMPILATIONS_OPTIMIZE)

    # Set excludes
    excludes: list[dict[str, str]] = [
        x for x in config.user_gui_settings if 'exclude' in x and x != {'exclude': ''}
    ]

    if excludes:
        exclude = excludes[0]['exclude']
        if 'a' in exclude:
            main_window.ui.checkBoxGlobalExcludeApplications.setChecked(True)
        if 'A' in exclude:
            main_window.ui.checkBoxGlobalExcludeAudio.setChecked(True)
        if 'b' in exclude:
            main_window.ui.checkBoxGlobalExcludeBadDumps.setChecked(True)
        if 'B' in exclude:
            main_window.ui.checkBoxGlobalExcludeBIOS.setChecked(True)
        if 'c' in exclude:
            main_window.ui.checkBoxGlobalExcludeCoverdiscs.setChecked(True)
        if 'D' in exclude:
            main_window.ui.checkBoxGlobalExcludeAddOns.setChecked(True)
        if 'd' in exclude:
            main_window.ui.checkBoxGlobalExcludeDemos.setChecked(True)
        if 'e' in exclude:
            main_window.ui.checkBoxGlobalExcludeEducational.setChecked(True)
        if 'f' in exclude:
            main_window.ui.checkBoxGlobalExcludeAftermarket.setChecked(True)
        if 'g' in exclude:
            main_window.ui.checkBoxGlobalExcludeGames.setChecked(True)
        if 'k' in exclude:
            main_window.ui.checkBoxGlobalExcludeMIA.setChecked(True)
        if 'm' in exclude:
            main_window.ui.checkBoxGlobalExcludeManuals.setChecked(True)
        if 'M' in exclude:
            main_window.ui.checkBoxGlobalExcludeMultimedia.setChecked(True)
        if 'o' in exclude:
            main_window.ui.checkBoxGlobalExcludeBonusDiscs.setChecked(True)
        if 'p' in exclude:
            main_window.ui.checkBoxGlobalExcludePirate.setChecked(True)
        if 'P' in exclude:
            main_window.ui.checkBoxGlobalExcludePreproduction.setChecked(True)
        if 'r' in exclude:
            main_window.ui.checkBoxGlobalExcludePromotional.setChecked(True)
        if 'u' in exclude:
            main_window.ui.checkBoxGlobalExcludeUnlicensed.setChecked(True)
        if 'v' in exclude:
    ...
    # Set up the paths interactivity
    main_window.ui.buttonGlobalChooseOutput.clicked.connect(
        lambda: set_path(
            main_window,
            main_window.output_folder,
            main_window.ui.labelGlobalOutputFolder,
            'output_folder',
        )
    )
    main_window.ui.checkBoxGlobalReplaceInputDats.clicked.connect(
        lambda: disable_incompatible_element(
            ...
            (
                main_window.ui.buttonGlobalChooseOutput,
                main_window.ui.labelGlobalSelectOutput,
                main_window.ui.labelGlobalOutputFolder,
            ),
        )
    )
    ...
    main_window.ui.buttonGlobalLanguageAllLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalSelectedLanguages,
            main_window.ui.listWidgetGlobalAvailableLanguages,
            all_items=True,
        )
    )
    main_window.ui.buttonGlobalLanguageAllRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalAvailableLanguages,
            main_window.ui.listWidgetGlobalSelectedLanguages,
            all_items=True,
    ...
    main_window.ui.buttonGlobalLanguageDown.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetGlobalSelectedLanguages, 'down')
    )
    main_window.ui.buttonGlobalLanguageLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalSelectedLanguages,
            main_window.ui.listWidgetGlobalAvailableLanguages,
        )
    )
    main_window.ui.buttonGlobalLanguageRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalAvailableLanguages,
            main_window.ui.listWidgetGlobalSelectedLanguages,
        )
    )
    main_window.ui.buttonGlobalLanguageUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetGlobalSelectedLanguages, 'up')
    )

    main_window.ui.buttonGlobalRegionAllLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalSelectedRegions,
            main_window.ui.listWidgetGlobalAvailableRegions,
            all_items=True,
        )
    )
    main_window.ui.buttonGlobalRegionAllRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalAvailableRegions,
            main_window.ui.listWidgetGlobalSelectedRegions,
            all_items=True,
    ...
    main_window.ui.buttonGlobalRegionDown.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetGlobalSelectedRegions, 'down')
    )
    main_window.ui.buttonGlobalRegionLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalSelectedRegions,
            main_window.ui.listWidgetGlobalAvailableRegions,
        )
    )
    main_window.ui.buttonGlobalRegionRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalAvailableRegions,
            main_window.ui.listWidgetGlobalSelectedRegions,
        )
    )
    main_window.ui.buttonGlobalRegionUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetGlobalSelectedRegions, 'up')
    )

    main_window.ui.buttonGlobalLocalizationAllLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalLocalizationSelectedLanguages,
            main_window.ui.listWidgetGlobalLocalizationAvailableLanguages,
            all_items=True,
        )
    )
    main_window.ui.buttonGlobalLocalizationAllRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalLocalizationAvailableLanguages,
            main_window.ui.listWidgetGlobalLocalizationSelectedLanguages,
            all_items=True,
    ...
    main_window.ui.buttonGlobalLocalizationDown.clicked.connect(
        lambda: order_list_items(
            main_window.ui.listWidgetGlobalLocalizationSelectedLanguages, 'down'
        )
    )
    main_window.ui.buttonGlobalLocalizationLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalLocalizationSelectedLanguages,
            main_window.ui.listWidgetGlobalLocalizationAvailableLanguages,
        )
    )
    main_window.ui.buttonGlobalLocalizationRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetGlobalLocalizationAvailableLanguages,
            main_window.ui.listWidgetGlobalLocalizationSelectedLanguages,
        )
    )
    main_window.ui.buttonGlobalLocalizationUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetGlobalLocalizationSelectedLanguages, 'up')
    )

    main_window.ui.buttonGlobalDefaultRegionOrder.clicked.connect(
        lambda: default_english_order(main_window, region_order_default, 'global')
    )

    # Set up the video order interactivity
    main_window.ui.buttonGlobalVideoStandardDown.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetGlobalVideoStandards, 'down')
    )
    main_window.ui.buttonGlobalVideoStandardUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetGlobalVideoStandards, 'up')
    )

    # Set up the excludes interactivity
    global_exclude_checkboxes = main_window.ui.tabGlobalExclusions.findChildren(
        qtw.QCheckBox, qtc.QRegularExpression('checkBoxGlobalExclude.*')
    )

    global_unlicensed_checkboxes = main_window.ui.tabGlobalExclusions.findChildren(
        qtw.QCheckBox,
    ...
    def clicked_unlicensed_all() -> None:
        ...
        if main_window.ui.checkBoxGlobalExcludeUnlicensedAll.isChecked():
            ...
            select_checkboxes(global_unlicensed_checkboxes, False)

    main_window.ui.checkBoxGlobalExcludeUnlicensedAll.clicked.connect(
        lambda: clicked_unlicensed_all()
    )

    main_window.ui.buttonGlobalDeselectAllExclude.clicked.connect(
        lambda: select_checkboxes(global_exclude_checkboxes, False)
    )
    main_window.ui.buttonGlobalSelectAllExclude.clicked.connect(
        lambda: select_checkboxes(global_exclude_checkboxes, True)
    )

    # Set up the options interactivity
    main_window.ui.checkBoxGlobalOptionsDisable1G1R.clicked.connect(
        lambda: disable_incompatible_checkbox(
            main_window.ui.checkBoxGlobalOptionsDisable1G1R,
            (main_window.ui.checkBoxGlobalOptionsLegacy,),
            (main_window.ui.checkBoxGlobalOptionsSplitRegions,),
        )
    )
    main_window.ui.checkBoxGlobalOptionsLegacy.clicked.connect(
        lambda: disable_incompatible_checkbox(
            main_window.ui.checkBoxGlobalOptionsLegacy,
            (
                main_window.ui.checkBoxGlobalOptionsDisable1G1R,
    ...
    main_window.ui.checkBoxGlobalOptionsSplitRegions.clicked.connect(
        lambda: disable_incompatible_checkbox(
            main_window.ui.checkBoxGlobalOptionsSplitRegions,
            (main_window.ui.checkBoxGlobalOptionsLegacy,),
            (main_window.ui.checkBoxGlobalOptionsDisable1G1R,),
    ...
    main_window.ui.checkBoxGlobalOptions1G1RNames.clicked.connect(
        lambda: show_hide(
            main_window.ui.checkBoxGlobalOptions1G1RNames,
    ...
    main_window.ui.actionCloneListUpdates.triggered.connect(
        lambda: write_config(
            main_window,
            dat_details,
            config,
    ...
    main_window.ui.actionSettings.triggered.connect(
        lambda: SettingsWindow(dat_details, config, main_window).exec()
    )
    ...
    main_window.ui.buttonAddDats.clicked.connect(
        lambda: add_list_items(
            main_window.ui.listWidgetOpenFiles, dat_details, config, main_window, 'files'
        )
    )
    main_window.ui.buttonAddFolder.clicked.connect(
        lambda: add_list_items(
            main_window.ui.listWidgetOpenFiles, dat_details, config, main_window, 'folder'
        )
    )
    main_window.ui.buttonAddFolderRecursive.clicked.connect(
        lambda: add_list_items(
            main_window.ui.listWidgetOpenFiles,
            dat_details,
            config,
            main_window,
            'folder',
    ...
    def quick_import_folder(
        ...
        if not main_window.quick_import_folder:
            msg = qtw.QMessageBox()
            msg.setText(
                'You haven\'t set a quick import folder yet. After you do, you can click the\n'
                'the quick import button to recursively import DAT files from your quick\n'
                'import folder without having to browse for it.\n\n'
                'Do you want to set a quick import folder now?\n'
            )
            msg.setWindowTitle('Set a quick import folder')
            ...
            set_up_folder: int = msg.exec()

            if set_up_folder == qtw.QMessageBox.Yes:  # type: ignore
                SettingsWindow(dat_details, config, main_window).exec()
        else:
            add_list_items(
                main_window.ui.listWidgetOpenFiles,
                dat_details,
    ...
    main_window.ui.buttonQuickImport.clicked.connect(
        lambda: quick_import_folder(dat_details, config, main_window)
    )
    main_window.ui.buttonClearDats.clicked.connect(
        lambda: remove_list_items(
            main_window.ui.listWidgetOpenFiles,
            dat_details,
            main_window.ui.labelSystemSettings,
            main_window,
        )
    )
    main_window.ui.buttonDeleteDats.clicked.connect(
        lambda: remove_list_items(
            main_window.ui.listWidgetOpenFiles,
            dat_details,
            main_window.ui.labelSystemSettings,
    ...
    main_window.ui.listWidgetOpenFiles.dropped.connect(
        lambda: add_list_items(
            main_window.ui.listWidgetOpenFiles, dat_details, config, main_window, 'dropped'
        )
    )

    # Set up the "Process DAT files" button
    main_window.ui.buttonGo.clicked.connect(
        lambda: write_config(
            main_window, dat_details, config, settings_window=None, run_retool=True
        )
    )
    ...
    def stop_threads() -> None:
        ...
        main_window.ui.buttonStop.setText(
            qtc.QCoreApplication.translate('MainWindow', 'Stopping...', None)
        )
...
def init_gui_system(
    ...
    main_window.ui.labelSystemOutputFolder.setText(
        qtc.QCoreApplication.translate('MainWindow', output_not_found, None)
    )
    main_window.ui.labelSystemCloneList.setText(
        qtc.QCoreApplication.translate('MainWindow', clone_list_not_found, None)
    )
    ...
    system_exclude_checkboxes = main_window.ui.tabSystemExclusions.findChildren(
        qtw.QCheckBox, qtc.QRegularExpression('checkBoxSystemExclude.*')
    )

    # Get the system options checkboxes
    system_options_checkboxes = main_window.ui.tabSystemOptions.findChildren(
        qtw.QCheckBox, qtc.QRegularExpression('checkBoxSystemOptions.*')
    )
    ...
    def system_settings(open_files_list: Any, config: Config) -> None:
        ...
        try:
            if open_files_list.selectedItems()[0].text() != 'No DAT files added yet':
                config.system_name = dat_details[open_files_list.selectedItems()[0].text()][
                    'system_name'
                ]
                main_window.ui.labelSystemSettings.setText(
                    f'These settings are only for <b>{config.system_name}</b>.'
                )
            else:
                return
        except Exception:
        ...
        select_checkboxes([main_window.ui.checkBoxSystemReplaceInputDats], False)
        select_checkboxes(system_exclude_checkboxes, False)
        select_checkboxes(system_options_checkboxes, False)

        main_window.ui.comboBoxSystemChooseCompilationsMode.setCurrentIndex(0)
        ...
        if not pathlib.Path(system_config_path).is_file():
            try:
                generate_config(
                    system_config_path,
        ...
        system_enable(
            ...
            [
                main_window.ui.buttonSystemChooseOutput,
                main_window.ui.labelSystemSelectOutput,
                main_window.ui.labelSystemOutputFolder,
                main_window.ui.buttonSystemChooseCloneList,
                main_window.ui.labelSystemSelectCloneList,
        ...
        if config.system_exclusions_options:
            if 'replace' in config.system_exclusions_options:
                main_window.ui.checkBoxSystemReplaceInputDats.setChecked(True)
                main_window.ui.buttonSystemChooseOutput.setEnabled(False)
                main_window.ui.buttonSystemClearOutput.setEnabled(False)
                main_window.ui.labelSystemSelectOutput.setEnabled(False)
                main_window.ui.labelSystemOutputFolder.setEnabled(False)
        ...
        region_order_user: list[str] = [
            x for x in config.region_order_user if x != 'United Kingdom'
        ]
        region_order_default: list[str] = [
            x for x in config.region_order_default if x != 'United Kingdom'
        ]
        config.system_region_order_user = [
            x for x in config.system_region_order_user if x != 'United Kingdom'
        ]

        if config.system_region_order_user:
            main_window.ui.listWidgetSystemSelectedRegions.addItems(
                [
                    str(x)
            ...
            main_window.ui.listWidgetSystemAvailableRegions.addItems(
                [x for x in region_order_default if x not in config.system_region_order_user]
            )
        else:
            main_window.ui.checkBoxSystemOverrideRegions.setChecked(False)
            main_window.ui.listWidgetSystemSelectedRegions.addItems(list(region_order_user))
            main_window.ui.listWidgetSystemAvailableRegions.addItems(
                [x for x in region_order_default if x not in region_order_user]
            )
        ...
        if config.system_languages_user_found:
            ...
            main_window.ui.listWidgetSystemAvailableLanguages.addItems(
                sorted([x for x in config.languages if x not in system_languages_user])
            )
        else:
            main_window.ui.checkBoxSystemOverrideLanguages.setChecked(False)

            main_window.ui.listWidgetSystemAvailableLanguages.addItems(sorted(config.languages))
        ...
        if config.system_localization_order_user:
            main_window.ui.listWidgetSystemLocalizationSelectedLanguages.addItems(
                system_localization_user
            )
            main_window.ui.listWidgetSystemLocalizationAvailableLanguages.addItems(
                sorted([x for x in config.languages if x not in system_localization_user])
            )
        else:
        ...
        if config.system_video_order_user:
            main_window.ui.listWidgetSystemVideoStandards.addItems(
                [
                    str(x)
                    for x in config.system_video_order_user
                    if x != {'override': 'true'} and x != {'override': 'false'}
                ]
            )
        else:
            main_window.ui.listWidgetSystemVideoStandards.setEnabled(False)
            main_window.ui.listWidgetSystemVideoStandards.addItems(list(config.video_order_default))
        ...
        system_enable(
            main_window.ui.checkBoxSystemOverrideOptions, [main_window.ui.scrollAreaSystemOptions]
        )
        ...
        if config.system_exclusions_options:
            ...
            if 'z' in config.system_exclusions_options:
                main_window.ui.checkBoxSystemOptionsModernPlatforms.setChecked(True)
            if 'y' in config.system_exclusions_options:
                main_window.ui.checkBoxSystemOptionsDemoteUnlicensed.setChecked(True)
            if 'nooverrides' in config.system_exclusions_options:
                main_window.ui.checkBoxSystemOptionsDisableOverrides.setChecked(True)
            if 'removesdat' in config.system_exclusions_options:
            ...
            # Show the associated lineEdit later, as it takes a while for the checkbox to be enabled
            if 'listnames' in config.system_exclusions_options:
                main_window.ui.checkBoxSystemOptions1G1RNames.setChecked(True)
            ...
            system_compilations_mode = [
                x for x in config.system_exclusions_options if 'compilations' in x
            ]

            if system_compilations_mode:
                system_compilations_option = system_compilations_mode[0]['compilations']

                if system_compilations_option == 'i':
                    main_window.ui.comboBoxSystemChooseCompilationsMode.setCurrentIndex(1)
                    main_window.ui.labelSystemCompilationsExplanation.setText(
                        const.COMPILATIONS_INDIVIDUAL
                    )
                elif system_compilations_option == 'k':
                    main_window.ui.comboBoxSystemChooseCompilationsMode.setCurrentIndex(2)
                    main_window.ui.labelSystemCompilationsExplanation.setText(
                        const.COMPILATIONS_KEEP
                    )
                elif system_compilations_option == 'o':
                    main_window.ui.comboBoxSystemChooseCompilationsMode.setCurrentIndex(3)
                    main_window.ui.labelSystemCompilationsExplanation.setText(
                        const.COMPILATIONS_OPTIMIZE
                    )

        system_excludes = [
            x for x in config.system_exclusions_options if 'exclude' in x and x != {'exclude': ''}
        ]

        if system_excludes:
            system_exclude = system_excludes[0]['exclude']
            if 'a' in system_exclude:
                main_window.ui.checkBoxSystemExcludeApplications.setChecked(True)
            if 'A' in system_exclude:
                main_window.ui.checkBoxSystemExcludeAudio.setChecked(True)
            if 'b' in system_exclude:
                main_window.ui.checkBoxSystemExcludeBadDumps.setChecked(True)
            if 'B' in system_exclude:
                main_window.ui.checkBoxSystemExcludeBIOS.setChecked(True)
            if 'c' in system_exclude:
                main_window.ui.checkBoxSystemExcludeCoverdiscs.setChecked(True)
            if 'D' in system_exclude:
                main_window.ui.checkBoxSystemExcludeAddOns.setChecked(True)
            if 'd' in system_exclude:
                main_window.ui.checkBoxSystemExcludeDemos.setChecked(True)
            if 'e' in system_exclude:
                main_window.ui.checkBoxSystemExcludeEducational.setChecked(True)
            if 'f' in system_exclude:
                main_window.ui.checkBoxSystemExcludeAftermarket.setChecked(True)
            if 'g' in system_exclude:
                main_window.ui.checkBoxSystemExcludeGames.setChecked(True)
            if 'k' in system_exclude:
                main_window.ui.checkBoxSystemExcludeMIA.setChecked(True)
            if 'm' in system_exclude:
                main_window.ui.checkBoxSystemExcludeManuals.setChecked(True)
            if 'M' in system_exclude:
                main_window.ui.checkBoxSystemExcludeMultimedia.setChecked(True)
            if 'o' in system_exclude:
                main_window.ui.checkBoxSystemExcludeBonusDiscs.setChecked(True)
            if 'p' in system_exclude:
                main_window.ui.checkBoxSystemExcludePirate.setChecked(True)
            if 'P' in system_exclude:
                main_window.ui.checkBoxSystemExcludePreproduction.setChecked(True)
            if 'r' in system_exclude:
                main_window.ui.checkBoxSystemExcludePromotional.setChecked(True)
            if 'u' in system_exclude:
                main_window.ui.checkBoxSystemExcludeUnlicensed.setChecked(True)
        ...
        if config.system_user_prefix:
            main_window.ui.lineEditSystemOptions1G1RPrefix.setText(config.system_user_prefix)
        if config.system_user_suffix:
            main_window.ui.lineEditSystemOptions1G1RSuffix.setText(config.system_user_suffix)
    ...
    main_window.ui.listWidgetOpenFiles.clicked.connect(
        lambda: system_settings(main_window.ui.listWidgetOpenFiles, config)
    )
    main_window.ui.listWidgetOpenFiles.keyPressed.connect(
        lambda: system_settings(main_window.ui.listWidgetOpenFiles, config)
    )
    ...
    main_window.ui.listWidgetOpenFiles.keyPressed.connect(
        lambda e: on_key(e, main_window.ui.listWidgetOpenFiles)
    )
    main_window.ui.listWidgetGlobalSelectedRegions.keyPressed.connect(
        lambda e: on_key(e, main_window.ui.listWidgetGlobalSelectedRegions)
    )
    main_window.ui.listWidgetGlobalSelectedLanguages.keyPressed.connect(
        lambda e: on_key(e, main_window.ui.listWidgetGlobalSelectedLanguages)
    )
    main_window.ui.listWidgetGlobalLocalizationSelectedLanguages.keyPressed.connect(
        lambda e: on_key(e, main_window.ui.listWidgetGlobalLocalizationSelectedLanguages)
    )
    main_window.ui.listWidgetSystemSelectedRegions.keyPressed.connect(
        lambda e: on_key(e, main_window.ui.listWidgetSystemSelectedRegions)
    )
    main_window.ui.listWidgetSystemSelectedLanguages.keyPressed.connect(
        lambda e: on_key(e, main_window.ui.listWidgetSystemSelectedLanguages)
    )
    main_window.ui.listWidgetSystemLocalizationSelectedLanguages.keyPressed.connect(
        lambda e: on_key(e, main_window.ui.listWidgetSystemLocalizationSelectedLanguages)
    )

    # Set up the buttons for the system paths
    main_window.ui.checkBoxSystemOverridePaths.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverridePaths,
            [
                main_window.ui.buttonSystemChooseOutput,
                main_window.ui.labelSystemSelectOutput,
    ...
    def clear_system_paths(clear_button: qtw.QPushButton) -> None:
        ...
        """
        if clear_button == main_window.ui.buttonSystemClearOutput:
            main_window.ui.labelSystemOutputFolder.setText(output_not_found)
            main_window.system_output_folder = ''
            config.system_output = ''
        elif clear_button == main_window.ui.buttonSystemClearCloneList:
            main_window.ui.labelSystemCloneList.setText(clone_list_not_found)
            main_window.system_clone_list = ''
            config.system_clone_list = ''
        elif clear_button == main_window.ui.buttonSystemClearMetadataFile:
    ...
    main_window.ui.buttonSystemChooseOutput.clicked.connect(
        lambda: set_path(
            ...
            main_window.system_output_folder,
            main_window.ui.labelSystemOutputFolder,
            'system_output_folder',
        )
    )
    main_window.ui.buttonSystemChooseCloneList.clicked.connect(
        lambda: set_path(
            main_window,
            main_window.system_clone_list,
            main_window.ui.labelSystemCloneList,
    ...
    main_window.ui.buttonSystemChooseMetadataFile.clicked.connect(
        lambda: set_path(
            main_window,
            main_window.system_metadata_file,
            main_window.ui.labelSystemMetadataFile,
    ...
    main_window.ui.buttonSystemClearCloneList.clicked.connect(
        lambda: clear_system_paths(main_window.ui.buttonSystemClearCloneList)
    )
    ...
    # Set up the regions/languages/localizations interactivity
    main_window.ui.checkBoxSystemOverrideLanguages.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverrideLanguages,
            [
                main_window.ui.buttonSystemLanguageAllLeft,
    ...
    main_window.ui.checkBoxSystemOverrideRegions.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverrideRegions,
            [
                main_window.ui.buttonSystemRegionAllLeft,
    ...
    main_window.ui.checkBoxSystemOverrideLocalization.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverrideLocalization,
            [
                main_window.ui.buttonSystemLocalizationAllLeft,
    ...
    main_window.ui.buttonSystemLanguageAllLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemSelectedLanguages,
            main_window.ui.listWidgetSystemAvailableLanguages,
            all_items=True,
    ...
    main_window.ui.buttonSystemLanguageAllRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemAvailableLanguages,
            main_window.ui.listWidgetSystemSelectedLanguages,
            all_items=True,
    ...
    main_window.ui.buttonSystemLanguageDown.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetSystemSelectedLanguages, 'down')
    )
    main_window.ui.buttonSystemLanguageLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemSelectedLanguages,
            main_window.ui.listWidgetSystemAvailableLanguages,
        )
    )
    main_window.ui.buttonSystemLanguageRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemAvailableLanguages,
            main_window.ui.listWidgetSystemSelectedLanguages,
        )
    )
    main_window.ui.buttonSystemLanguageUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetSystemSelectedLanguages, 'up')
    )

    main_window.ui.buttonSystemRegionAllLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemSelectedRegions,
            main_window.ui.listWidgetSystemAvailableRegions,
            all_items=True,
    ...
    main_window.ui.buttonSystemRegionAllRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemAvailableRegions,
            main_window.ui.listWidgetSystemSelectedRegions,
            all_items=True,
    ...
    main_window.ui.buttonSystemRegionDown.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetSystemSelectedRegions, 'down')
    )
    main_window.ui.buttonSystemRegionLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemSelectedRegions,
            main_window.ui.listWidgetSystemAvailableRegions,
        )
    )
    main_window.ui.buttonSystemRegionRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemAvailableRegions,
            main_window.ui.listWidgetSystemSelectedRegions,
        )
    )
    main_window.ui.buttonSystemRegionUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetSystemSelectedRegions, 'up')
    )

    main_window.ui.buttonSystemLocalizationAllLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemLocalizationSelectedLanguages,
            main_window.ui.listWidgetSystemLocalizationAvailableLanguages,
            all_items=True,
    ...
    main_window.ui.buttonSystemLocalizationAllRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemLocalizationAvailableLanguages,
            main_window.ui.listWidgetSystemLocalizationSelectedLanguages,
            all_items=True,
    ...
    main_window.ui.buttonSystemLocalizationDown.clicked.connect(
        lambda: order_list_items(
            main_window.ui.listWidgetSystemLocalizationSelectedLanguages, 'down'
        )
    )
    main_window.ui.buttonSystemLocalizationLeft.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemLocalizationSelectedLanguages,
            main_window.ui.listWidgetSystemLocalizationAvailableLanguages,
        )
    )
    main_window.ui.buttonSystemLocalizationRight.clicked.connect(
        lambda: move_list_items(
            main_window.ui.listWidgetSystemLocalizationAvailableLanguages,
            main_window.ui.listWidgetSystemLocalizationSelectedLanguages,
        )
    )
    main_window.ui.buttonSystemLocalizationUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetSystemLocalizationSelectedLanguages, 'up')
    )

    main_window.ui.buttonSystemDefaultRegionOrder.clicked.connect(
        lambda: default_english_order(main_window, region_order_default, 'system')
    )

    # Set up the video order interactivity
    main_window.ui.checkBoxSystemOverrideVideo.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverrideVideo,
            [
                main_window.ui.buttonSystemVideoStandardDown,
    ...
    main_window.ui.buttonSystemVideoStandardDown.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetSystemVideoStandards, 'down')
    )
    main_window.ui.buttonSystemVideoStandardUp.clicked.connect(
        lambda: order_list_items(main_window.ui.listWidgetSystemVideoStandards, 'up')
    )

    # Set up the excludes interactivity
    main_window.ui.checkBoxSystemOverrideExclusions.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverrideExclusions,
            [
                *system_exclude_checkboxes,
    ...
    system_unlicensed_checkboxes = main_window.ui.tabSystemExclusions.findChildren(
        qtw.QCheckBox,
    ...
    def clicked_unlicensed_all() -> None:
        ...
        if main_window.ui.checkBoxSystemExcludeUnlicensedAll.isChecked():
            ...
            select_checkboxes(system_unlicensed_checkboxes, False)

    main_window.ui.checkBoxSystemExcludeUnlicensedAll.clicked.connect(
        lambda: clicked_unlicensed_all()
    )

    main_window.ui.buttonSystemDeselectAllExclude.clicked.connect(
        lambda: select_checkboxes(system_exclude_checkboxes, False)
    )
    main_window.ui.buttonSystemSelectAllExclude.clicked.connect(
        lambda: select_checkboxes(system_exclude_checkboxes, True)
    )

    # Set up the options interactivity
    main_window.ui.checkBoxSystemOverrideOptions.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverrideOptions, [main_window.ui.scrollAreaSystemOptions]
        )
    )

    main_window.ui.checkBoxSystemOptionsDisable1G1R.clicked.connect(
        lambda: disable_incompatible_checkbox(
            main_window.ui.checkBoxSystemOptionsDisable1G1R,
            (main_window.ui.checkBoxSystemOptionsLegacy,),
            (main_window.ui.checkBoxSystemOptionsSplitRegions,),
        )
    )
    main_window.ui.checkBoxSystemOptionsLegacy.clicked.connect(
        lambda: disable_incompatible_checkbox(
            main_window.ui.checkBoxSystemOptionsLegacy,
            (
                main_window.ui.checkBoxSystemOptionsDisable1G1R,
    ...
    main_window.ui.checkBoxSystemOptionsSplitRegions.clicked.connect(
        lambda: disable_incompatible_checkbox(
            main_window.ui.checkBoxSystemOptionsSplitRegions,
            (main_window.ui.checkBoxSystemOptionsLegacy,),
            (main_window.ui.checkBoxSystemOptionsDisable1G1R,),
    ...
    main_window.ui.checkBoxSystemOptions1G1RNames.clicked.connect(
        lambda: show_hide(
            main_window.ui.checkBoxSystemOptions1G1RNames,
            main_window.ui.frameSystemOptions1G1RPrefix,
        )
    ...
    main_window.ui.checkBoxSystemOptionsTrace.clicked.connect(
        lambda: show_hide(
            main_window.ui.checkBoxSystemOptionsTrace, main_window.ui.frameSystemOptionsTrace
        )
    )

    # Set up the post filter interactivity
    main_window.ui.checkBoxSystemOverridePostFilter.clicked.connect(
        lambda: system_enable(
            main_window.ui.checkBoxSystemOverridePostFilter,
            [main_window.ui.textEditSystemFilterInclude],
        )
